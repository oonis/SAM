...
import static ij.measure.Measurements.MODE;
...
public final class ThresholdAdjuster extends PlugInDialog implements PlugIn, Measurements,
    ...
    static int mode = RED;  
    static final String[] methodNames = AutoThresholder.getMethods();
...
//                int max = (int)ip.getMaxThreshold();
                if (cal.isSigned16Bit()) {
                    int min = (int)cal.getCValue(level1);
                    int max = (int)cal.getCValue(level2);
                    if (Recorder.scriptMode())
...
//                }
    ...
    void runThresholdCommand() {
        listener.thresholdsSet();
//        System.out.println(minThreshold+"\t"+maxThreshold);
...
//            (new Thresholder()).run("mask");
    ...
    public void setListener(ThresholdListener lis) {
        listener = lis;
    }
    ...
    /**
     * @return <code>String</code> The current method ("Default", "Huang", etc).
     */
    ...
    /**
     * @return <code>String</code> The current mode ("Red","B&W" or"Over/Under").
     */
